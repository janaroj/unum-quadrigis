...
/**
 * Encapsulates everything that has to do with the tab (the tab labelled
 * "History" in the menu).
 */
public class HistoryTab {
	...
	private Component drawHistoryMainPane() {
		...
		table.getSelectionModel().addListSelectionListener(
				new ListListener(table, model, domainController));
		JScrollPane scrollPane = new JScrollPane(table);
...
class ListListener implements ListSelectionListener {
	...
	public void valueChanged(ListSelectionEvent e) {
		...
		if (table.getSelectedRow() >= 0) {
			if (model.getSecondHistoryTableModel().getRowCount() != 0)
				model.getSecondHistoryTableModel().removeItems();

			List<SoldItem> soldItems = domainController.loadBoughtItems(row);

			for (SoldItem si : soldItems) {
				model.getSecondHistoryTableModel().addItem(si);
			}
